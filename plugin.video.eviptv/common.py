...
def TextBoxesPlain(announce):
	class TextBox():
		...
		def setControls(self):
			self.win.getControl(self.CONTROL_LABEL).setLabel('[COLOR powderblue]Evolution IPTV[/COLOR]') # set heading
...
def TextBoxes(announce):
	class TextBox():
		...
		def __init__(self,*args,**kwargs):
			...
			self.setControls()
		def setControls(self):
			self.win.getControl(self.CONTROL_LABEL).setLabel('Evolution IPTV Log Viewer') # set heading
			try: f=open(announce); text=f.read()
			except: text=announce
			self.win.getControl(self.CONTROL_TEXTBOX).setText(str(text))
			return
