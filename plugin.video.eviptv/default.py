...
def peamenyy(params):
    ...
    if channels == 1 and GoDev.mode != 5 and GoDev.mode != 1:
        ...
        if not xbmc.getCondVisibility('Pvr.HasTVChannels'):
            plugintools.add_item( action=vod_channels("R29EZXYuY29ycmVjdFBWUg=="),   title="Best IPTV IN TOWN!" , thumbnail=os.path.join(LOAD_LIVE,vod_channels("bGl2ZXR2LnBuZw==")), fanart=os.path.join(LOAD_LIVE,vod_channels("YmFja2dyb3VuZC5wbmc=")) ,  folder=False )
