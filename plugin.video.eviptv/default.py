...
def peamenyy(params):
    ...
    if channels == 1 and GoDev.mode != 5 and GoDev.mode != 1:
        ...
        plugintools.add_item( action=vod_channels("c2VjdXJpdHlfY2hlY2s="),  title="[COLOR gold][B][I]LIVE TV[/I][/B][/COLOR]", thumbnail=os.path.join(LOAD_LIVE,vod_channels("bGl2ZXR2LnBuZw==")) , fanart=os.path.join(LOAD_LIVE,vod_channels("YmFja2dyb3VuZC5wbmc=")) ,  folder=True )
        ...
        if not xbmc.getCondVisibility('Pvr.HasTVChannels'):
            plugintools.add_item( action=vod_channels("R29EZXYuY29ycmVjdFBWUg=="),   title="[COLOR red][B]Setup Full PVR (In BETA)[/B][/COLOR]" , thumbnail=os.path.join(LOAD_LIVE,vod_channels("bGl2ZXR2LnBuZw==")), fanart=os.path.join(LOAD_LIVE,vod_channels("YmFja2dyb3VuZC5wbmc=")) ,  folder=False )
            ...
            plugintools.addItem('[COLOR lime][B]Launch PVR[/B][/COLOR]','speed',11,GoDev.Images + 'logo.png',GoDev.Images + 'background.png')
...
def security_check(params):
	...
	for channel in tree.findall(sync_data("Y2hhbm5lbA==")):
		...
		if 'CANADA' in kanalinimi.upper():
			ICON=os.path.join(LOAD_LIVE,sync_data("Y2FuZGEuanBn"))
		...
		if 'INDIAN' in kanalinimi.upper():
			ICON=os.path.join(LOAD_LIVE,sync_data("aWRpZGFuaGEucG5n"))
		...
		if 'PREMIUM MOVIES' in kanalinimi.upper():
			ICON=os.path.join(LOAD_LIVE,sync_data("bW92aWUtdGltZS1maW5hbDIuanBn"))
		if 'SOCCER - MATCHCENTER' in kanalinimi.upper():
			ICON=os.path.join(LOAD_LIVE,sync_data("MmVkZGUuanBn"))
		if 'EUROPE ENTERTAINMENT' in kanalinimi.upper():
			ICON=os.path.join(LOAD_LIVE,sync_data("ZXVyb2ZsYWcxMzIzLmpwZw=="))
		if 'SPECIAL EVENTS' in kanalinimi.upper():
			ICON=os.path.join(LOAD_LIVE,sync_data("c3BlY2lhbGV2ZW50c3MucG5n"))
		if 'NORTH AMERICA SPORTS' in kanalinimi.upper():
			ICON=os.path.join(LOAD_LIVE,sync_data("d3IzNDM0cmY0cjM0ZGZlM3JvLnBuZw=="))
		if 'EUROPE SPORTS' in kanalinimi.upper():
			ICON=os.path.join(LOAD_LIVE,sync_data("d3IzNDM0cmY0cjM0ZGZlM3JvLnBuZw=="))
		if 'SPORTS PACKAGE (DIGITAL)' in kanalinimi.upper():
			ICON=os.path.join(LOAD_LIVE,sync_data("d3IzNDM0cmY0cjM0ZGZlM3JvLnBuZw=="))
		if 'CARIBBEAN' in kanalinimi.upper():
			ICON=os.path.join(LOAD_LIVE,sync_data("MTMyMzIxMzIzMjEzMjMyMTMyMzIxMzIzMjEzMjMyMTMyMzIxMzIzMi5wbmc="))
